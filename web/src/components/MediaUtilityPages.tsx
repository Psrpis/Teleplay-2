...
import { BarChart3, CalendarDays, Clock3, FolderPlus, HardDrive, Menu, Search, Trash2 } from 'lucide-react';
import { useSearchParams } from 'react-router-dom';
import { useBulkAddToCollection, useClearHistory, useCollections, useCreateCollection, useDeleteHistory, useFavorites, useHistory, useMediaSearch, useMediaStats, useStorageStats, formatDuration, formatFileSize } from '../lib/api';
...
export function SearchPage() {
    ...
    return <Shell title="Search" eyebrow="Find something to watch"><form onSubmit={(event) => { event.preventDefault(); setParams(fileType ? { q: query, type: fileType } : { q: query }); }} className="relative max-w-2xl"><Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-dark-600" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} className="input w-full py-4 pl-12 text-lg" placeholder="Search titles, files, and metadata…" /></form><div className="mt-10">{isLoading ? <p className="text-dark-400">Searching your library…</p> : (query || fileType) && data?.files.length ? <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">{data.files.map((file) => <MediaCard key={file.id} file={file} grid />)}</div> : <div className="rounded-2xl border border-dashed border-white/10 p-10 text-center text-dark-500">{query || fileType ? 'No results found.' : 'Start typing to search your private library.'}</div>}</div></Shell>;
}
...
function FavoriteGrid({ isLoading, files }: { isLoading: boolean; files: any[] }) { return <div>{isLoading ? <p className="text-dark-400">Loading favorites…</p> : files.length ? <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">{files.map((file) => <MediaCard key={file.id} file={file} grid />)}</div> : <div className="rounded-2xl border border-dashed border-white/10 p-12 text-center text-dark-500">No favorites yet. Use the heart on a media card to save one.</div>}</div>; }
